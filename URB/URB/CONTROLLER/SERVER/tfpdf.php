...
require_once dirname(__FILE__) . '/../../MODEL/BL/Result.php';

class tFPDF {

	var $validObject;        // The constructor will set the Result object in case of error 
	var $unifontSubset;
	...
	*******************************************************************************/
	function __construct($orientation='P', $unit='mm', $size='A4') {
		$this->validObject = new Result();
	
		// Some checks
		$this->_dochecks();
		if (($this->validObject)->errorCode == 0) {
			...
			if      ($unit=='pt') {
				...
				($this->validObject)->errorCode = 1;
				($this->validObject)->errorDescription = 'Incorrect unit: ' . $unit;
			}
			...
			if (($this->validObject)->errorCode == 0) {
				...
				if($orientation=='p' || $orientation=='portrait') {
					...
					($this->validObject)->errorCode = 2;
					($this->validObject)->errorDescription = 'Incorrect orientation: ' . $orientation;
				}
	...
	function setDisplayMode($zoom, $layout='default') {
		...
		if($zoom=='fullpage' || $zoom=='fullwidth' || $zoom=='real' || $zoom=='default' || !is_string($zoom)) {
			...
			($this->validObject)->errorCode = 1;
			($this->validObject)->errorDescription = 'Incorrect zoom display mode: ' . $zoom;
		}
		...
		if (($this->validObject)->errorCode == 0) {
			if($layout=='single' || $layout=='continuous' || $layout=='two' || $layout=='default') {
				...
				($this->validObject)->errorCode = 1;
				($this->validObject)->errorDescription = 'Incorrect layout display mode: ' . $layout;
			}
		}
	}
...
function _dochecks()
{
	...
	if(sprintf('%.1F', 1.0) != '1.0') {
		...
		($this->validObject)->errorCode = 1;
		($this->validObject)->errorDescription = 'This version of PHP is not supported';
	}
	...
	if(!function_exists('mb_strlen')) {
		...
		($this->validObject)->errorCode = 2;
		($this->validObject)->errorDescription = 'mbstring extension is not available';
	}

	// Check mbstring overloading
	if(ini_get('mbstring.func_overload') & 2) {
		// $this->Error('mbstring overloading must be disabled');
		($this->validObject)->errorCode = 3;
		($this->validObject)->errorDescription = 'mbstring overloading must be disabled';
	}
