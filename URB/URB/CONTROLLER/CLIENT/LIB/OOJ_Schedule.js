 ...
 * @return {OOJ_Schedule}
 */
 var OOJ_Schedule = OOJ_Widget.extend({

	/**
	 * Constructor.
	 * 
	 * @method init
	 * @param       {string}    sID             The ID of the element that should be used to display the calendar.
	 * @param       {Object}    oOptions        All the configurable options that should be used (refer to "Options" documentation for properties).
...
function calendarJs( id, options, startDateTime ) {
	...
	function buildDateHeader() {
		...
		previousMonthButton = createElement('div');
		previousMonthButton.innerHTML = '<i class="fa fa-chevron-circle-left" aria-hidden="true"></i>';
		previousMonthButton.style.cssFloat = 'left';
		...
		todayButton = createElement('div');
		todayButton.innerHTML = '<i class="fa fa-thumb-tack" aria-hidden="true"></i>';
		todayButton.style.cssFloat = 'left';
		...
		refreshButton = createElement('div');
		...
		searchButton = createElement("div");
		...
		nextMonthButton = createElement('div');
		nextMonthButton.innerHTML = '<i class="fa fa-chevron-circle-right" aria-hidden="true"></i>';
		nextMonthButton.style.cssFloat = 'right';
		...
		if ( _options.manualEditingEnabled ) {
			addEventButton = createElement('div');
			addEventButton.innerHTML = '<i class="fa fa-plus" aria-hidden="true"></i>';
			addEventButton.style.cssFloat = 'right';
		...
		if ( _options.exportEventsEnabled ) {
			...
			_element_HeaderDateDisplay_ExportEventsButton.innerHTML = '<i class="fa fa-download" aria-hidden="true"></i>';
			_element_HeaderDateDisplay_ExportEventsButton.style.cssFloat = 'right';
		...
		listAllEventsButton.innerHTML = '<i class="fa fa-eye" aria-hidden="true"></i>';
		listAllEventsButton.style.cssFloat = 'right';
		...
		listWeekEventsButton.innerHTML = '<i class="fa fa-bars" aria-hidden="true"></i>';
		listWeekEventsButton.style.cssFloat = 'right';
		...
		if ( _options.fullScreenModeEnabled ) {
			_element_HeaderDateDisplay_FullScreenButton = createElement('div');
			_element_HeaderDateDisplay_FullScreenButton.innerHTML = '<i class="fa fa-expand" aria-hidden="true"></i>';
			_element_HeaderDateDisplay_FullScreenButton.style.cssFloat = 'right';
	...
	function buildFullDayView() {
		...
		titleBar = createElement( "div", "title-bar" );
		...
		previousMonthButton = createElement('div');
		previousMonthButton.innerHTML = '<i class="fa fa-chevron-circle-left" aria-hidden="true"></i>';
		previousMonthButton.style.cssFloat = 'right';
		...
		if ( _options.exportEventsEnabled ) {
			...
			_element_FullDayView_ExportEventsButton.innerHTML = '<i class="fa fa-external-link-square" aria-hidden="true"></i>';
			_element_FullDayView_ExportEventsButton.style.cssFloat = 'right';
		...
		if ( _options.fullScreenModeEnabled ) {
			_element_FullDayView_FullScreenButton = createElement('div');
			_element_FullDayView_FullScreenButton.innerHTML = '<i class="fa fa-expand" aria-hidden="true"></i>';
			_element_FullDayView_FullScreenButton.style.cssFloat = 'right';
	...
	function buildDefaultOptions( newOptions ) {
		...
		if ( !isDefined( _options.nextDayTooltipText ) ) {
			_options.nextDayTooltipText = "Siguiente día";
		}
		...
		if ( !isDefined( _options.nextWeekTooltipText ) ) {
			_options.nextWeekTooltipText = "Siguiente semana";
		}

		if ( !isDefined( _options.addEventTooltipText ) ) {
			_options.addEventTooltipText = "Añadir evento";
		}
		...
		if ( !isDefined( _options.expandDayTooltipText ) ) {
			_options.expandDayTooltipText = "Desplegar día";
		}

		if ( !isDefined( _options.listAllEventsTooltipText ) ) {
			_options.listAllEventsTooltipText = "Ver todos los eventos";
		}

		if ( !isDefined( _options.listWeekEventsTooltipText ) ) {
			_options.listWeekEventsTooltipText = "Ver eventos de esta semana";
		}

		if ( !isDefined( _options.dayHeaderNames ) ) {
			_options.dayHeaderNames = [ "Lun", "Mar", "Mie", "Jue", "Vie", "Sab", "Dom" ];
		}

		if ( !isDefined( _options.dayNames ) ) {
			_options.dayNames = [ "Lunes", "Martes", "Miercoles", "Jueves", "Viernes", "Sabado", "Domingo" ];
		}
		...
		if ( !isDefined( _options.isAllDayText ) ) {
			_options.isAllDayText = "Es dia completo";
		}
		...
		if ( !isDefined( _options.removeEventText ) ) {
			_options.removeEventText = "Quitar";
		}
		...
		if ( !isDefined( _options.exportStartFilename ) ) {
			_options.exportStartFilename = "EVENTOS_";
		}

		if ( !isDefined( _options.fromTimeErrorMessage ) ) {
			_options.fromTimeErrorMessage = "Seleccione una hora válida para 'Desde'.";
		}

		if ( !isDefined( _options.toTimeErrorMessage ) ) {
			_options.toTimeErrorMessage = "Seleccione una hora válida para 'Hasta'.";
		}
		...
		if ( !isDefined( _options.titleErrorMessage ) ) {
			_options.titleErrorMessage = "Introduzca un valor para el campo título.";
		}
		...
		if ( !isDefined( _options.allDayText ) ) {
			_options.allDayText = "Día completo";
		}
		...
		if ( !isDefined( _options.startsOnText ) ) {
			_options.startsOnText = "Comienza el";
		}

		if ( !isDefined( _options.andFinishesOnText ) ) {
			_options.andFinishesOnText = "y acaba el";
		}
		...
		if ( !isDefined( _options.confirmEventRemoveTitle ) ) {
			_options.confirmEventRemoveTitle = "Confirme el borrado";
		}

		if ( !isDefined( _options.confirmEventRemoveMessage ) ) {
			_options.confirmEventRemoveMessage = "El borrado de este evento no puede deshacerse. ¿Continuar?";
		}
		...
		if ( !isDefined( _options.selectExportTypeTitle ) ) {
			_options.selectExportTypeTitle = "Tipo de exportación";
		}
		...
		if ( !isDefined( _options.selectColorsText ) ) {
			_options.selectColorsText = "Selección de colores";
		}
		...
		if ( !isDefined( _options.matchCaseText ) ) {
			_options.matchCaseText = "Coincidir may/min";
		}
		...
		if ( !isDefined( _options.repeatsText ) ) {
			_options.repeatsText = "Se repite:";
		}

		if ( !isDefined( _options.repeatDaysToExcludeText ) ) {
			_options.repeatDaysToExcludeText = "Repetir días a excluir:";
		}

		if ( !isDefined( _options.daysToExcludeText ) ) {
			_options.daysToExcludeText = "Días a excluir:";
		}
		...
		if ( !isDefined( _options.repeatsEveryDayText ) ) {
			_options.repeatsEveryDayText = "Cada día";
		}
		...
		if ( !isDefined( _options.repeatOptionsTitle ) ) {
			_options.repeatOptionsTitle = "Opciones de repetición";
		}

		if ( !isDefined( _options.moreText ) ) {
			_options.moreText = "Mas";
		}
		...
		if ( !isDefined( _options.removeAllEventsInSeriesText ) ) {
			_options.removeAllEventsInSeriesText = "Borrar todos los eventos en la serie";
		}
		...
		if ( !isDefined( _options.organizerNameText ) ) {
			_options.organizerNameText = "Organizador:";
		}
		
		if ( !isDefined( _options.organizerEmailAddressText ) ) {
			_options.organizerEmailAddressText = "Correo del organizador::";
		}

		if ( !isDefined( _options.enableFullScreenTooltipText ) ) {
			_options.enableFullScreenTooltipText = "Pantalla completa On";
		}

		if ( !isDefined( _options.disableFullScreenTooltipText ) ) {
			_options.disableFullScreenTooltipText = "Pantalla completa Off";
		}
		...
		if ( !isDefined( _options.noEventsAvailableText ) ) {
			_options.noEventsAvailableText = "No hay eventos disponibles";
		}

		if ( !isDefined( _options.viewWeekEventsText ) ) {
			_options.viewWeekEventsText = "Ver eventos de la semana";
		}
	}
