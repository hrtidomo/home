...
var OOJ_Crypt = Class.extend({
	...
	init: function(options) {
		...
		opts = this.extendObject(this.defaults, options);

		if        (opts.method == 'md5') {
			this.sEncripted = this.md5(opts);
		} else if (opts.method == 'sha1') {
			this.sEncripted = this.sha1(opts);
		} else if (opts.method == 'b64enc') {
			this.sEncripted = this.b64enc(opts);
		} else if (opts.method == 'b64dec') {
			this.sEncripted = this.b64dec(opts);
		} else if (opts.method == 'xteaenc') {
			this.sEncripted = this.xteaenc(opts);
		} else if (opts.method == 'xteadec') {
			this.sEncripted = this.xteadec(opts);
		} else if (opts.method == 'xteab64enc') {
			var tmpenc = this.xteaenc(opts);
			opts.method = "b64enc";
			opts.source = tmpenc;
			this.sEncripted = this.b64enc(opts);
		} else if (opts.method == 'xteab64dec') {
			var tmpdec = this.b64dec(opts);
			opts.method = "xteadec";
			opts.source = tmpdec;
			this.sEncripted = this.xteadec(opts);
		}
	...
	md5:function(params) {
		...
		function core_hmac_md5(key, data) {
			...
			bKey= str2binl(key);
			if(bkey.length > 16) {
				bkey = core_md5(bkey, key.length * params.chrsz);
			}
			...
			hash = core_md5(ipad.concat(str2binl(data)), 512 + data.length * params.chrsz);
		...
		function md5_cmn(q, a, b, x, s, t) {
			return oTHIS.safe_add(bit_rol(oTHIS.safe_add(oTHIS.safe_add(a, q), oTHIS.safe_add(x, t)), s),b);
		}
		...
		function core_md5(x, len) {
			...
			for(var i = 0; i < x.length; i += 16) {
				...
				a = oTHIS.safe_add(a, olda);
				b = oTHIS.safe_add(b, oldb);
				c = oTHIS.safe_add(c, oldc);
				d = oTHIS.safe_add(d, oldd);
		  }
		...
		function core_sha1(x, len) {
			...
			for(var i = 0; i < x.length; i += 16) {
				...
				for(var j = 0; j < 80; j++) {
					...
					var t = oTHIS.safe_add(oTHIS.safe_add(rol(a, 5), sha1_ft(j, b, c, d)), oTHIS.safe_add(oTHIS.safe_add(e, w[j]), sha1_kt(j)));
					e = d;
					d = c;
					c = rol(b, 30);
					b = a;
	...
	xteadec:function(params) {
		var v = new Array(2), k = new Array(4), s = "", i;

		for (var i=0; i<4; i++)
			k[i] = Str4ToLong(params.strKey.slice(i*4,(i+1)*4));
